...
# ==============================================================================
# MUGEN/IKEMEN GO Character Manager v2.2 - Ultimate Debug Edition
# Added a master error handler to catch silent crashes.
# ==============================================================================

# --- Master Error Handler ---
def log_error_and_exit(e):
    """Writes an exception to a log file and exits."""
    base_path = get_base_path()
    log_file_path = os.path.join(base_path, 'crash_log.txt')
    print(f"\nFATAL ERROR: A critical error occurred. Please check the 'crash_log.txt' file for details.")
    with open(log_file_path, 'w') as f:
        f.write("MUGEN Manager v2.2 Crash Report\n")
        f.write("=================================\n\n")
...
# ... (I've included the full script below for simplicity) ...
...
def load_or_create_config(config_path):
    ...
    try:
        with open(config_path, 'r') as f:
            ...
            if not is_valid: raise KeyError("One or more keys are missing from config.json")
            return config_data
...
def read_roster(roster_path, engine_type):
    ...
    try:
        if engine_type.upper() == "IKEMEN":
            with open(roster_path, 'r', encoding='utf-8') as f: config_data = json.load(f)
...
def add_to_roster_ikemen(roster_path, char_folder_name, def_file_name):
    ...
    if "Characters" not in config_data or not isinstance(config_data["Characters"], list): config_data["Characters"] = []
...
def main_loop():
    ...
    if not roster_path or not os.path.exists(roster_path):
        print(f"\nERROR: Roster file not found for '{ENGINE_TYPE}' at '{GAME_PATH}'. Check config.json.");
    ...
    while True:
        ...
        elif choice == '3': delete_character(current_roster, roster_path, ENGINE_TYPE, CHARS_FOLDER)
